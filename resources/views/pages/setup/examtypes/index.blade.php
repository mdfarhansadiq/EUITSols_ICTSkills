                ...
                <div class="card">
                    <div class="card-header">
                        <span class="float-left">
                            <h4>View Exam Types</h4>
                        </span>
                        <span class="float-right">
                            <a href="{{ route('examtypes.create') }}" class="btn btn-info">Add new type</a>
                        </span>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table table-responsive">
                            <table id="table" class="">
                                ...
                                <tbody>
                                    @forelse($exam_types as $key => $et)

                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $et->name }}</td>
                                            <td>{{ date('d-m-Y', strtotime($et->created_at)) }}</td>
                                            <td>{{ $et->created_user->name ?? 'system' }}</td>
                                            <td class="text-middle py-0 align-middle">
                                                <div class="btn-group">
                                                    <a href="javascript:void(0)" class="btn btn-info btnView"
                                                        data-id="{{ $et->id }}"><i class="fas fa-eye"></i></a>

                                                    <a href="{{ route('examtypes.edit', $et->id) }}"
                                                        class="btn btn-dark btnEdit"><i class="fas fa-edit"></i></a>


                                                    <a href="{{ route('examtypes.destroy', $et->id) }}" class="btn btn-danger btnDelete"><i class="fas fa-trash"></i></a>
        ...
        $(document).ready(function() {
            ...
            $('#table').on('click', 'tbody tr td .btn-group .btnView', function () {
                if ($(this).data('id') != null || $(this).data('id') != '') {
                    let url = ("{{ route('examtypes.show', ['id']) }}");
