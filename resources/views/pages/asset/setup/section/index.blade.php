...
@section('title', 'Asset Management - Section')
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <span class="float-right">
                        @if(Auth::user()->can('add section') || Auth::user()->role->id == 1)<a href="{{ route('asset.setup.section.create') }}" class="btn btn-info">Add new section</a>@endif
                    </span>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                                @foreach ( $sections as $key=>$section)
                                   <tr>
                                    ...
                                    <td>{{$section->created_user->name}}</td>
                                    <td>{{date('d-m-Y',strtotime($section->created_user->created_at))}}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="javascript:void(0)" class="btn btn-info btnView"
                                            data-id="{{ $section->id }}"><i class="fas fa-eye"></i></a>
                                            @if(Auth::user()->can('edit section') || Auth::user()->role->id == 1)
                                                <a href="{{ route('asset.setup.section.edit', $section->id) }}" class="btn btn-dark btnEdit"><i class="fas fa-edit"></i></a>
                                            @endif
                                            @if(Auth::user()->can('delete section') || Auth::user()->role->id == 1)
                                                <a href="{{ route('asset.setup.section.destroy', $section->id) }}" class="btn btn-danger btnDelete"><i class="fas fa-trash"></i></a>
                                            @endif
                                        </div>
                                    </td>
                                   </tr>
        ...
        $(document).ready(function() {
            ...
            $('.btnView').click( function(){
                if($(this).data('id') != null || $(this).data('id') != ''){
                    let url = ("{{ route('asset.setup.section.show', ['id']) }}");
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $('#view-department').html(response.department.department_name);
